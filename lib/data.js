...
function miniBoards(mini) {
  ...
  push('cover', mini.cover);
  for (const img of mini.gallery) push('gallery', img);
  for (const img of mini.process) push('process', img);
  return boards;
}

// One artboard per image (cover, then gallery, then process) across every
// mini in the room. Rooms hold few minis but many photos, so per-mini
// boards would give 1-board corridors.
export function getRoomBoards(slug) {
  return getRoomMinis(slug).flatMap(miniBoards);
}

// Museum-specific room listing: same rooms as getRooms(), with a board
// count instead of a preview image.
export function getMuseumRooms() {
  return getRooms().map((room) => ({
    name: room.name,
    slug: room.slug,
    boardCount: getRoomBoards(room.slug).length,
    miniCount: room.count,
  }));
}
