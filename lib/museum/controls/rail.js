...
// The rail glides on a fixed-duration eased tween rather than an
// exponential lerp. A lerp never actually arrives: it crawls asymptotically
// into the stop, so the last stretch reads as drift rather than as arriving,
// and a second step landing during that tail visibly snaps. A timed tween
// starts, eases, and finishes.
...
// Two taps on the same artwork within this window opens it at full
// resolution. Long enough for a deliberate double-tap on a phone, short
// enough that two separate looks don't trigger it.
const DOUBLE_TAP_MS = 320;

export function createRailControls({ camera, canvas, layout, onOpenCard, onCloseCard, onTapZonesVisible, onZoom }) {
  let cursor = 0;
  let idle = 0;
  let yaw = 0;
  let tYaw = 0;
  ...
  // Where the current tween started, so it can interpolate from a true
  // origin instead of chasing a moving target.
  const fromPos = new THREE.Vector3();
  const fromLook = new THREE.Vector3();
  let transit = 1; // 0..1 through the current glide; 1 means settled
  ...
  function setCursor(i, instant = false) {
    // The room is a ring, so the cursor wraps instead of clamping: stepping
    // past the last artwork arrives back at the first, and no room dead-ends.
  ...
  function onPointerUp(e) {
    ...
    // A second tap on the same artwork opens it full-resolution. Checked
    // before the normal tap so a double-tap never also steps the rail.
    const slot = pick(e.clientX, e.clientY);
    const now = performance.now();
    if (slot?.board && slot === lastTapSlot && now - lastTapAt < DOUBLE_TAP_MS) {
      lastTapAt = 0;
      lastTapSlot = null;
      idle = 0;
      onZoom?.(slot);
  ...
  return {
    ...
    update(dt) {
      ...
      if (transit < 1) {
        transit = Math.min(1, transit + dt / TRANSIT_SECONDS);
        // easeInOutCubic: leaves and arrives at rest, so neither end jerks.
        const e = transit < 0.5 ? 4 * transit ** 3 : 1 - (-2 * transit + 2) ** 3 / 2;
        camPos.copy(fromPos).lerp(tgtPos, e);
        camLook.copy(fromLook).lerp(tgtLook, e);
      } else {
        camPos.copy(tgtPos);
        camLook.copy(tgtLook);
      }
