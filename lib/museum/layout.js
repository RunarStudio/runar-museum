...
export function layout(boards) {
  ...
  const slots = [];
  for (let i = 0; i < n; i++) {
    // Negative winding so that advancing the cursor carries the visitor to
    // their right. Flipping the ring here rather than swapping the arrow
    // buttons keeps everything downstream consistent: the counter still
    // climbs as you travel right, and auto-advance still drifts that way too.
    const angle = -i * step;
