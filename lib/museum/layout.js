...
export const SPAN = 4.6;
// Distance from the centerline to each wall's board plane, in meters. Set
// wide deliberately: a narrow corridor makes the rail feel like a tunnel,
// while a wide hall reads as walking a circuit around an open room.
const WALL_OFFSET = 3.4;
// Board (and camera stop) height off the floor, in meters.
const BOARD_Y = 1.6;
// Boards per corridor leg before it turns 90°. 4 facing pairs.
const WRAP_AFTER = 8;
...
export function layout(boards) {
  ...
  while (pairsWalked < totalPairs) {
    ...
    // Leg length grows every two turns (1,1,2,2,3,3,... × PAIRS_PER_LEG) —
    // the classic square-spiral construction. A constant leg length would
    // close into a square after 4 turns and retrace itself exactly; this
    // widens each loop so a large room spirals outward without a slot
    // ever landing on top of an earlier one. The first leg (by far the
    // common case today) is unaffected: it's always exactly PAIRS_PER_LEG.
    const legUnits = Math.floor(legIdx / 2) + 1;
    const legPairs = Math.min(legUnits * PAIRS_PER_LEG, totalPairs - pairsWalked);
