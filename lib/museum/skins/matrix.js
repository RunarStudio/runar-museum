...
const RAIN_RING_DEPTH = 150; // how far out the storm extends
...
export const matrix = {
  ...
  build(layout, root, quality) {
    ...
    for (let i = 0; i < sheetCount; i++) {
      ...
      // sqrt() makes the sample uniform by AREA rather than by radius. A
      // plain random() clusters everything near the room, which is exactly
      // what made the storm read flat — most of its volume is far out.
      const dist = radius + RAIN_RING_CLEARANCE + Math.sqrt(Math.random()) * RAIN_RING_DEPTH;
      const depthT = Math.min(1, (dist - radius) / RAIN_RING_DEPTH);
      sheets.push({
        ...
        // Distant sheets are physically larger so they still subtend a
        // useful angle, and dimmer so distance reads as distance. Together
        // these are what turn a flat curtain into depth.
        spread: 1 + depthT * 2.6,
        dim: 1 - depthT * 0.72,
      });
    }
    ...
    function layoutSheets() {
      for (let i = 0; i < sheets.length; i++) {
        ...
        scale.set(sh.spread, sh.stretch * (1 + sh.spread * 0.25), 1);
        m4.compose(pos, quat, scale);
        rainField.setMatrixAt(i, m4);
      }
      rainField.instanceMatrix.needsUpdate = true;
    }
    // Per-instance tint multiplies the shared material colour, so the cycling
    // base colour still applies on top of each sheet's own distance dimming.
    const shade = new THREE.Color();
    for (let i = 0; i < sheets.length; i++) {
      shade.setScalar(sheets[i].dim);
      rainField.setColorAt(i, shade);
    }
    if (rainField.instanceColor) rainField.instanceColor.needsUpdate = true;
    ...
    // No per-slot marker posts: they existed to echo the dungeon's pilasters
    // back when the corridor needed something to divide it, but on a ring the
    // glow ring already reads as the room's edge, and a post directly behind
    // an artwork looked like a stand holding the picture up.
    for (const s of layout.slots) {
      anchors.push([s.pos[0], 2.4, s.pos[2]]);
    }

    // Floor glow: one continuous ring at the base of the wall. With no walls
    // or ceiling, this is the only thing that says where the room ends and
    // the endless grid begins — and as a ring it draws the circuit the rail
    // actually walks.
    const glowMat = new THREE.MeshBasicMaterial({
      color: 0x3ad6a6,
      transparent: true,
      // Additive on a near-black floor blooms hard: at full strength this
      // read as a solid bar across the screen rather than an edge marker.
      opacity: 0.32,
      blending: THREE.AdditiveBlending,
      depthWrite: false,
      fog: false,
    ...
    const glowRing = new THREE.Mesh(new THREE.RingGeometry(radius + 0.5, radius + 0.66, 96), glowMat);
